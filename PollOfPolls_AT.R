...
dta <- read.csv("at-6.csv",na.strings = "Not Available")

dt <- dta %>%
  select(Polling.Firm, Fieldwork.End, Sample.Size, ÖVP, SPÖ, FPÖ, NEOS, GRÜNE) %>%
  pivot_longer(cols = c(ÖVP, SPÖ, FPÖ, NEOS, GRÜNE), names_to = 'Partito') %>%
  select(Partito, Fieldwork.End, value, Polling.Firm, Sample.Size)

colnames(dt) <- c("Partito", "Data", "Percentuale", "Istituto", "Campione")

dt$Data <- as.Date(dt$Data, format = "%Y-%m-%d")  # RESTA Date!!

dt$Percentuale <- as.numeric(gsub("%", "", dt$Percentuale))

dt <- dt %>%
  arrange(Data, Partito, Istituto)

write.csv(dt,'DatiAustria.csv')

DT::datatable(dt %>% mutate(Data = format(Data, "%d/%m/%Y")))
...
dates <- tibble(Data = seq(dt$Data[1], dt$Data[nrow(dt)], by = "day"))
